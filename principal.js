document.addEventListener("DOMContentLoaded", () => {
  ...
  // Función para cargar y mostrar un modal (se mantiene para otros usos si los hubiera)
  async function loadModal(containerId, htmlPath, jsPath, initFunction) {
    const container = document.getElementById(containerId)
    if (!container) {
      console.error(`Contenedor no encontrado: ${containerId}`)
      returndocument.addEventListener("DOMContentLoaded", () => {
  const botonAgregar = document.getElementById("botonAgregar")
  const menuOpciones = document.getElementById("menuOpciones")
  const btnNuevoIngreso = document.getElementById("btnNuevoIngreso")
  const btnNuevaDeuda = document.getElementById("btnNuevaDeuda")
  ...
  // Estos ahora redirigen a la página correspondiente y abren el formulario
